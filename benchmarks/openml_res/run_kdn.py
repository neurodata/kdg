...
def experiment(dataset_id, layer_size=1000, reps=5, random_state=42):
    ...
    for train_sample in train_samples:
        for rep in range(reps):
            ...
            nn = getNN(input_size=X_train.shape[1], num_classes=np.max(
                y_train)+1, layer_size=layer_size)
            uncalibrated_nn = KerasClassifier(model=nn)
            history = uncalibrated_nn.fit(
                X_train, keras.utils.to_categorical(y_train), **fit_kwargs)

            
            model_kdn = kdn(network=uncalibrated_nn.model)
            model_kdn.fit(X_train, y_train, X_cal, y_cal)
            model_kdn.global_bias = -1e200
            proba_kdn = model_kdn.predict_proba(X_test)
            proba_kdn_geod = model_kdn.predict_proba(
                X_test, distance='Geodesic')
            ...
            calibrated_nn_isotonic = calcv(
                uncalibrated_nn, method='isotonic', ensemble=False, cv='prefit')
            calibrated_nn_isotonic.fit(X_cal, y_cal)

            calibrated_nn_sigmoid = calcv(
                uncalibrated_nn, method='sigmoid', ensemble=False, cv='prefit')
