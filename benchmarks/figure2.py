...
for ii, model in enumerate(models):
    for jj, simulation in enumerate(simulations):
        ...
        ax[jj][ii*3+0].plot(sample_size, df[model_key+'geod_med'], c=clr, linewidth=linewidth[0], label=model.upper()+'-Geodesic')
        ax[jj][ii*3+0].plot(sample_size, df[model_key+'med'], c=clr, linewidth=linewidth[0], label=model.upper()+'-Euclidean', linestyle='dashed')
        ax[jj][ii*3+0].plot(sample_size, df[parent_key+'med'], c="k", label=parent.upper())
        ax[jj][ii*3+0].fill_between(sample_size, df[model_key+'25'], df[model_key+'75'], facecolor=clr, alpha=.3)
        ax[jj][ii*3+0].fill_between(sample_size, df[model_key+'geod_25'], df[model_key+'geod_75'], facecolor=clr, alpha=.3)
        ...
        if jj==0:
            ax[jj][ii*3+1].set_title(model.upper() + ' and '+parent.upper(), fontsize=labelsize+30)
        ...
        ax[jj][ii*3+2].plot(r, np.array(df[model_key+'geod_med']).ravel(), c=clr, linewidth=linewidth[0], label=model.upper()+'-Geodesic')
        ax[jj][ii*3+2].plot(r, np.array(df[model_key+'med']).ravel(), c=clr, linewidth=linewidth[0], label=model.upper()+'-Euclidean', linestyle='dashed')
        ax[jj][ii*3+2].plot(r, np.array(df[parent_key+'med']).ravel(), c="k", label=parent.upper())
        ax[jj][ii*3+2].fill_between(r, np.array(df[model_key+'25']).ravel(), np.array(df[model_key+'75']).ravel(), facecolor=clr, alpha=.3)
        ax[jj][ii*3+2].fill_between(r, np.array(df[model_key+'geod_25']).ravel(), np.array(df[model_key+'geod_75']).ravel(), facecolor=clr, alpha=.3)
        ax[jj][ii*3+2].fill_between(r, np.array(df[parent_key+'25']).ravel(), np.array(df[parent_key+'75']).ravel(), facecolor='k', alpha=.3)
